...
export default function ProfileContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // validation
    if (!formData.name.trim()) { setStatus("Please enter your name."); return; }
    if (!formData.skillLevel) { setStatus("Please choose a skill level."); return; }
    if (!/^\d{4}$/.test(formData.postcode.trim())) { setStatus("Enter a valid 4-digit postcode."); return; }
