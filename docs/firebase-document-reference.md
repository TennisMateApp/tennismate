...
| `events/{eventId}` | Created in `app/events/new/page.tsx` or `components/events/DesktopCreateEventPage.tsx` when a host publishes an event. | Social tennis event listing. | Event metadata: `hostId`, `visibility`, `title`, `type`, `location`, `court`, `start`, `end`, canonical `durationMinutes`, temporary compatibility mirror `durationMins`, `timeZone`, `description`, skill range labels, `spotsTotal`, `spotsFilled`, `status`, `createdAt`, `updatedAt`. |
