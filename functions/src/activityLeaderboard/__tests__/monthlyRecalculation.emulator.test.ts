...
function updateMillis(snapshot: FirebaseFirestore.DocumentSnapshot): number { assert.ok(snapshot.updateTime); return snapshot.updateTime.toMillis(); }
...
test("same run attempt is idempotent and does not create another audit", async () => {
  ...
  assert.equal(retry.generationId, first.generationId); assert.equal(afterRequest.data()?.attempts, 1); assert.equal(updateMillis(afterRequest), updateMillis(beforeRequest)); assert.equal(updateMillis(afterAudit), updateMillis(beforeAudit)); assert.equal((await db.collection("activity_phase2_runs").get()).size, 1); assert.equal(afterAudit.data()?.triggerType, "pilot");
});
